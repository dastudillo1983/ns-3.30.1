 ...
 */
#ifndef LRWPAN_AODVROUTINGPROTOCOL_H
#define LRWPAN_AODVROUTINGPROTOCOL_H
 ...
 */
class AODVLrWpanRoutingProtocol : public LrWpanRoutingProtocol
{
public:
  /**
  ...
  static const uint32_t LRWPAN_AODV_SAP;

  /// constructor
  AODVLrWpanRoutingProtocol ();
  virtual ~AODVLrWpanRoutingProtocol ();
  virtual void DoDispose ();

  // Inherited from Ipv4RoutingProtocol
  Ptr<LrWpanRoute> RouteOutput (Ptr<Packet> p, const LrWpanMacHeader &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr);
  bool RouteInput (Ptr<const Packet> p, const LrWpanMacHeader &header, Ptr<const NetDevice> idev,
                   UnicastForwardCallback ucb, ErrorCallback ecb);
 //virtual void SetLrWpan (Ptr<LrWpan> lrwpan);
...
private:
  ...
  LrWpanRequestQueue m_queue;
  /// Broadcast ID
  uint32_t m_requestId;
  /// Request sequence number
  uint32_t m_seqNo;
  /// Handle duplicated RREQ
  LrWpanIdCache m_rreqIdCache;
  /// Handle duplicated broadcast/multicast packets
  LrWpanDuplicatePacketDetection m_dpd;
  /// Handle neighbors
  LrWpanNeighbors m_nb;
  ...
  // Packet, destination Mac16Address, SAP
  TracedCallback<Ptr<const Packet>,Mac16Address,uint8_t> m_send;
...
private:
   ...
   */ 
  void DeferredRouteOutput (Ptr<const Packet> p, const LrWpanMacHeader & header, UnicastForwardCallback ucb, ErrorCallback ecb);
  /**
   ...
   */ 
  bool Forwarding (Ptr<const Packet> p, const LrWpanMacHeader & header, UnicastForwardCallback ucb, ErrorCallback ecb);
  /**
   ...
   */
  void ScheduleRreqRetry (Mac16Address dst);
  /**
   ...
   */
  void ProcessHello (LrWpanRrepHeader const & rrepHeader, Mac16Address receiverIfaceAddr);

  ///\name Receive control packets
  //\{
  /// Receive and process control packet
  bool RecvAodv (Ptr<NetDevice> device, Ptr<const Packet> packet,
			uint16_t protocolNumber, const Address &from);
  /// Receive RREQ
  void RecvRequest (Ptr<Packet> p, Mac16Address receiver, Mac16Address src);
  /// Receive RREP
  void RecvReply (Ptr<Packet> p, Mac16Address my,Mac16Address src);
  /// Receive RREP_ACK
  void RecvReplyAck (Mac16Address neighbor);
  /// Receive RERR from node with address src
  void RecvError (Ptr<Packet> p, Mac16Address src);
  //\}

  ///\name Send
  //\{
  /// Forward packet from route request queue
  void SendPacketFromQueue (Mac16Address dst, Ptr<LrWpanRoute> route);
  /// Send hello
  void SendHello ();
  /// Send RREQ
  void SendRequest (Mac16Address dst);
  /// Send RREP
  void SendReply (LrWpanRreqHeader const & rreqHeader, LrWpanRoutingTableEntry const & toOrigin);
  /** Send RREP by intermediate node
   * \param toDst routing table entry to destination
   * \param toOrigin routing table entry to originator
   * \param gratRep indicates whether a gratuitous RREP should be unicast to destination
   */
  void SendReplyByIntermediateNode (LrWpanRoutingTableEntry & toDst, LrWpanRoutingTableEntry & toOrigin, bool gratRep);
  /// Send RREP_ACK
  void SendReplyAck (Mac16Address neighbor);
  /// Initiate RERR
  void SendRerrWhenBreaksLinkToNextHop (Mac16Address nextHop);
  /// Forward RERR
  void SendRerrMessage (Ptr<Packet> packet,  std::vector<Mac16Address> precursors);
  /**
   ...
   */
  void SendRerrWhenNoRouteToForward (Mac16Address dst, uint32_t dstSeqNo, Mac16Address origin);
  /// @}
   ...
   */
  void SendTo (Ptr<Packet> packet, Mac16Address destination);
  ...
  std::map<Mac16Address, Timer> m_addressReqTimer;
  /**
   * Handle route discovery process
   * \param dst the destination IP address
   */
  void RouteRequestTimerExpire (Mac16Address dst);
  /**
   ...
   */
  void AckTimerExpire (Mac16Address neighbor, Time blacklistTimeout);
...
#endif /* LRWPAN_AODVROUTINGPROTOCOL_H */
