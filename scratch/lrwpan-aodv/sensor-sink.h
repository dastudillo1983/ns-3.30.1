 ...
 * \defgroup SensorSink SensorSink
 *
 * This application was written to complement OnOffApplication, but it
 * is more general so a SensorSink name was selected.  Functionally it is
 ...
 */
class SensorSink : public Application
{
public:
  /**
  ...
  bool HandleRead (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocolNumber, const Address &from);
  //void Receive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocolNumber, const Address &source);
...
private:
  ...
  //NetDevice::ReceiveCallback m_receive;
  // In the case of TCP, each socket accept returns a new socket, so the 
  // listening socket is stored separately from the accepted sockets

  Address         m_local;        //!< Local address to bind to
  uint64_t        m_totalRx;      //!< Total bytes received
  uint8_t         m_sap;          //!< Service Access Point in LLC sublayer
