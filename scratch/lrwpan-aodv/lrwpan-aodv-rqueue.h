 ...
 */
#ifndef LRWPAN_AODV_RQUEUE_H
#define LRWPAN_AODV_RQUEUE_H
 ...
 */
class LrWpanQueueEntry
{
public:
  /// IPv4 routing unicast forward callback typedef
  typedef LrWpanRoutingProtocol::UnicastForwardCallback UnicastForwardCallback;
  /// IPv4 routing error callback typedef
  typedef LrWpanRoutingProtocol::ErrorCallback ErrorCallback;
  /**
   * constructor
   *
   * \param pa the packet to add to the queue
   * \param h the LrWpanMacHeader
   * \param ucb the UnicastForwardCallback function
   * \param ecb the ErrorCallback function
   * \param exp the expiration time
   */
  LrWpanQueueEntry (Ptr<const Packet> pa = 0, LrWpanMacHeader const & h = LrWpanMacHeader (),
   ...
   */
  bool operator== (LrWpanQueueEntry const & o) const
  {
    return ((m_packet == o.m_packet) && (m_header.GetShortDstAddr() == o.m_header.GetShortDstAddr ()) && (m_expire == o.m_expire));
  }
   ...
   */
  LrWpanMacHeader GetLrWpanMacHeader () const
  {
    return m_header;
  }
  /**
   ...
   */
  void SetLrWpanMacHeader (LrWpanMacHeader h)
  {
    m_header = h;
  }
  /**
...
public:
   ...
   */
  LrWpanRequestQueue (uint32_t maxLen, Time routeToQueueTimeout)
    : m_maxLen (maxLen),
   ...
   */
  bool Enqueue (LrWpanQueueEntry & entry);
  /**
   ...
   */
  bool Dequeue (Mac16Address dst, LrWpanQueueEntry & entry);
  /**
   * Remove all packets with destination IP address dst
   * \param dst the destination IP address
   */
  void DropPacketWithDst (Mac16Address dst);
  /**
...
private:
  /// The queue
  std::vector<LrWpanQueueEntry> m_queue;
   ...
   */
  void Drop (LrWpanQueueEntry en, std::string reason);
