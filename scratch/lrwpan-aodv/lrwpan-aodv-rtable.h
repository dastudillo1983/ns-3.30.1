 ...
 */
#ifndef LRWPAN_AODV_RTABLE_H
#define LRWPAN_AODV_RTABLE_H
...
public:
   ...
   */
  LrWpanRoutingTableEntry (Ptr<NetDevice> dev = 0,Mac16Address dst = Mac16Address (), bool vSeqNo = false, uint32_t seqNo = 0,
                     uint16_t  hops = 0,
                     Mac16Address nextHop = Mac16Address (), Time lifetime = Simulator::Now ());

  ~LrWpanRoutingTableEntry ();
   ...
   */
  bool InsertPrecursor (Mac16Address id);
  /**
   * Lookup precursor by address
   * \param id precursor address
   * \return true on success
   */
  bool LookupPrecursor (Mac16Address id);
  /**
   * \brief Delete precursor
   * \param id precursor address
   * \return true on success
   */
  bool DeletePrecursor (Mac16Address id);
   ...
   */
  void GetPrecursors (std::vector<Mac16Address> & prec) const;
  //\}
  ...
  {
    return m_lrWpanRoute->GetDestination ();
  }
  /**
   * Get route function
   * \returns The IPv4 route
   */
  Ptr<LrWpanRoute> GetRoute () const
  {
    return m_lrWpanRoute;
  }
  /**
   * Set route function
   * \param r the IPv4 route
   */
  void SetRoute (Ptr<LrWpanRoute> r)
  {
    m_lrWpanRoute = r;
  }
  /**
   ...
   */
  void SetNextHop (Mac16Address nextHop)
  {
   ...
   */
  Mac16Address GetNextHop () const
  {
    return m_lrWpanRoute->GetPANCoordinator ();
  }
  /**
   * Set output device
  ...
  {
    return m_lrWpanRoute->GetOutputDevice ();
  }
  /**
   ...
   */
  bool operator== (Mac16Address const  dst) const
  {
    return (m_lrWpanRoute->GetDestination () == dst);
  }
  /**
...
private:
  ...
  */
   ...
   *   - output device
   */
  Ptr<LrWpanRoute> m_lrWpanRoute;
  /// Routing flags: valid, invalid or in search
  RouteFlags m_flag;

  /// List of precursors
  std::vector<Mac16Address> m_precursorList;
...
public:
   ...
   */
  bool AddRoute (LrWpanRoutingTableEntry & r);
  /**
   * Delete routing table entry with destination address dst, if it exists.
   * \param dst destination address
   * \return true on success
   */
  bool DeleteRoute (Mac16Address dst);
  /**
   ...
   * \return true on success
   */
  bool LookupRoute (Mac16Address dst, LrWpanRoutingTableEntry & rt);
  /**
   ...
   * \return true on success
   */
  bool LookupValidRoute (Mac16Address dst, LrWpanRoutingTableEntry & rt);
  /**
   * Update routing table
   * \param rt entry with destination address dst, if exists
   * \return true on success
   */
  bool Update (LrWpanRoutingTableEntry & rt);
  /**
   ...
   * \return true on success
   */
  bool SetEntryState (Mac16Address dst, RouteFlags state);
  /**
   ...
   * \param unreachable
   */
  void GetListOfDestinationWithNextHop (Mac16Address nextHop, std::map<Mac16Address, uint32_t> & unreachable);
  /**
   ...
   */
  void InvalidateRoutesWithDst (std::map<Mac16Address, uint32_t> const & unreachable);
   ...
   * \return true on success
   */
  bool MarkLinkAsUnidirectional (Mac16Address neighbor, Time blacklistTimeout);
  /**
   * Print routing table
   * \param stream the output stream
   */
...
private:
  /// The routing table
  std::map<Mac16Address, LrWpanRoutingTableEntry> m_ipv4AddressEntry;
   ...
   */
  void Purge (std::map<Mac16Address, LrWpanRoutingTableEntry> &table) const;
};

}  // namespace aodv
}  // namespace ns3

#endif /* LRWPAN_AODV_RTABLE_H */
