...
Destination-Sequenced Distance Vector (DSDV6) routing protocol is a pro-active, table-driven routing protocol
...
`this URL <https://wiki.ittc.ku.edu/resilinets/ResiliNets_Publications#.E2.80.9CDestination-Sequenced_Distance_Vector_.28DSDV6.29_Routing_Protocol_Implementation_in_ns-3.E2.80.9D>`_. 

DSDV6 Routing Overview
*********************

DSDV6 Routing Table: Every node will maintain a table listing all the other nodes it has known either directly
...
The DSDV6 update message consists of three fields, Destination Address, Sequence Number and Hop Count.

Each node uses 2 mechanisms to send out the DSDV6 updates. They are,
...
2. Trigger Updates
    ...
    receives a DSDV6 packet that caused a change in its routing table. The original paper did not clearly mention
    when for what change in the table should a DSDV6 update be sent out. The current implemntation sends out an update
...
The current implementation covers all the above features of DSDV6. The current implementation also has a request queue
